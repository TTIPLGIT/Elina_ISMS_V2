  ...
  try {           
    ...
    $received_for = $inputArray['received_for'];
            ...
            if($source_type !=''){  
              ...
              if($user_id!=" " && $receipt_no1==" " && $receipt_no2==" ")
              {  if($source_type=="UAM"){
                  if($uam_action ==[]){
                   $rows=DB::select("select name,audit_logs.id,audit_action,description,action_date_time,role_name,'UAM' AS module_name from audit_logs inner join users on audit_logs.user_id =
                   users.id  where audit_table_name LIKE 'uam%' and user_id=$user_id");
                  }
                  else{
                    $rows=DB::select("select name,audit_logs.id,audit_action,description,action_date_time,role_name,'UAM' AS module_name from audit_logs inner join users on audit_logs.user_id =
                   users.id  where audit_table_name LIKE 'uam%' and user_id=$user_id and audit_action='$uam_action'");
                  }
              }
              }
              else if($user_id!=" " && $receipt_no1!=" " && $receipt_no2==" ")
              {  if($source_type=="UAM"){
                if($uam_action == " "){
               $rows=DB::select("SELECT name,audit_logs.id,audit_action,description,action_date_time,role_name,'UAM' AS module_name from audit_logs inner join users on audit_logs.user_id =
                   users.id where audit_table_name LIKE 'uam%' and user_id=$user_id and DATE_FORMAT(audit_logs.action_date_time,'%Y-%m-%d') ='".$receipt_no1."' ");
                }
                else{
                  $rows=DB::select("SELECT name,audit_logs.id,audit_action,description,action_date_time,role_name,'UAM' AS module_name from audit_logs inner join users on audit_logs.user_id =
                   users.id where audit_table_name LIKE 'uam%' and user_id=$user_id and DATE_FORMAT(audit_logs.action_date_time,'%Y-%m-%d') ='".$receipt_no1."' and  audit_action='$uam_action'");
                }
              }
              }
              else if($user_id!=" " && $receipt_no1!=" " && $receipt_no2 !=" ")
              {  $this->WriteFileLog("from both date only");
                if($source_type=="UAM"){
                  if($uam_action ==" "){
               $rows=DB::select("SELECT name,audit_logs.id,audit_action,description,action_date_time,role_name,'UAM' AS module_name from audit_logs inner join users on audit_logs.user_id =
                   users.id where audit_table_name LIKE 'uam%' and  user_id=$user_id and DATE_FORMAT(audit_logs.action_date_time,'%Y-%m-%d') >='".$receipt_no1."' and DATE_FORMAT(audit_logs.action_date_time,'%Y-%m-%d') <='".$receipt_no2."'");
                  }
                  else{
                    $rows=DB::select("SELECT name,audit_logs.id,audit_action,description,action_date_time,role_name,'UAM' AS module_name from audit_logs inner join users on audit_logs.user_id =
                    users.id where audit_table_name LIKE 'uam%' and  user_id=$user_id and DATE_FORMAT(audit_logs.action_date_time,'%Y-%m-%d') >='".$receipt_no1."' and DATE_FORMAT(audit_logs.action_date_time,'%Y-%m-%d') <='".$receipt_no2."'and  audit_action='$uam_action'");
                  }
                }
              }
              else{
               $rows=DB::select("SELECT * from audit_logs inner join users on audit_logs.user_id =
               users.id where user_id=$user_id and DATE_FORMAT(audit_logs.action_date_time,'%Y-%m-%d') >='".$receipt_no1."' and DATE_FORMAT(audit_logs.action_date_time,'%Y-%m-%d') <='".$receipt_no2."'");
           ...
           if($user_id!=" " && $receipt_no1==" " && $receipt_no2==" ")
           {  
                $rows=DB::select("select name,audit_logs.id,audit_action,description,action_date_time,role_name,upper(REPLACE(audit_table_name, '_', ' ' )) AS module_name from audit_logs inner join users on audit_logs.user_id =users.id  where user_id=$user_id");
           }
           else if($user_id!=" " && $receipt_no1!=" " && $receipt_no2==" ")
           {  
            $rows=DB::select("SELECT name,audit_logs.id,audit_action,description,action_date_time,role_name,upper(REPLACE(audit_table_name, '_', ' ' ))  AS module_name from audit_logs inner join users on audit_logs.user_id =
                users.id where user_id=$user_id and DATE_FORMAT(audit_logs.action_date_time,'%Y-%m-%d') ='".$receipt_no1."' ");
           }
           else if($user_id!=" " && $receipt_no1!=" " && $receipt_no2 !=" ")
           {  $this->WriteFileLog("from both date only");
            $rows=DB::select("SELECT name,audit_logs.id,audit_action,description,action_date_time,role_name,upper(REPLACE(audit_table_name, '_', ' ' ))  AS module_name from audit_logs inner join users on audit_logs.user_id =
