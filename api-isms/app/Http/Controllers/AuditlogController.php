  ...
  // echo json_encode($rows);exit;
      ...
      public function login_search(Request $request)
      {
        ...
        try {           
          ...
          {
           $rows=DB::select("SELECT us.name,us.email,date_format(fa.login_time,'%d-%m-%Y & %h:%i:%s') AS login_time,date_format(fa.logout_time,'%d-%m-%Y & %h:%i:%s') AS logout_time FROM login_audit AS fa INNER JOIN users us ON (us.id=fa.user_id)

            WHERE DATE_FORMAT(fa.login_time,'%Y-%m-%d') >='".$from_date1."' AND date_format(fa.login_time,'%Y-%m-%d')   <= '".$to_date1."' and us.email LIKE '".$user_id."%'
            UNION ALL
            SELECT '_'as name,mas.email,date_format(mas.attempt_time,'%d-%m-%Y & %h:%i:%s') AS login_time,'Unsuccessful Attempt' AS logout_time FROM mismatch_attempt_audit AS mas 
