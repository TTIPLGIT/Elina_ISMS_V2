...
public function storedata(Request $request)
{
  ...
  // $this->WriteFileLog($input['screen_id']);
              ...
              $this->admin_notification('UAM', 'Create' , 'uam_roles' , $role_id ,$input['role_name'] , ' UAM Role has been Successfully Created' );
    ...
    try {
      ...
      if (json_encode($role_name_check) == '[]') { 
          ...
          DB::transaction(function() use($input) {
              ...
              $this->admin_notification('UAM', 'Update' , 'uam_roles' , $input['role_id'] ,$input['role_name'] , ' UAM Role has been Successfully Updated' );
...
public function data_delete($id)
{
    try{
        ...
        //    $role_name_fetch=$role_name[0]->role_name;
           ...
           $this->auditLog('uam_roles', $id, 'Delete', 'Deleted the uam role', auth()->user()->id, NOW(),'');
           $serviceResponse = array();
           $serviceResponse['Code'] = config('setting.status_code.success');
           $serviceResponse['Message'] = config('setting.status_message.success');
           $serviceResponse['Data'] = 1;
