...
class FAQmodulesController extends BaseController
{
   ...
   public function get_data(Request $request)
   {
    try {
        ...
        $rows=DB::select('select `a`.* from `faq_module_name` as `a` where `a`.`status` = 0 ');
...
public function get_faq_data()
{
    try{
        ...
        $one_rows = DB::select("SELECT * FROM faq_module_name WHERE id IN (SELECT module_id FROM faq_ans) AND STATUS = 0 ORDER BY id ASC ");
...
public function get_search_ques(Request $request)
{
    try{
        ...
        $rows = DB::select("Select * from faq_module_name where status='0' and module_name LIKE '%". $input['module_name']."%'");
...
public function storedata(Request $request)
{
    ...
    $role_name_fetch=$role_name[0]->role_name;
            $this->auditLog('FAQ_modules', $uam_modules_id, 'Create', 'Created new FAQ module', auth()->user()->id, NOW(),$role_name_fetch);
        });
