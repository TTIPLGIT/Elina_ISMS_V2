...
class UamModulesController extends BaseController
{
    ...
    public function get_data(Request $request)
    {
        try {
            ...
            // ->get();
            $rows = DB::select('select `a`.`module_name` as `parent_module_name`, `b`.`module_name`, `b`.`display_order`, `b`.`module_id`, `a`.`module_id` as `moduleid` from `uam_modules` as `a` right join `uam_modules` as `b` on `b`.`parent_module_id` = `a`.`module_id` where `b`.`active_flag` = 0 AND b.parent_module_id !=0');
    ...
    public function get_uam_modules()
    {
        try {
            ...
            $rows = DB::select("SELECT * FROM uam_modules WHERE active_flag =0 AND parent_module_id !=0 AND module_type=1");
        ...
        try {
            ...
            if ($module_check == []) {
                ...
                DB::transaction(function () use ($input) {
                    ...
                    $this->admin_notification('UAM', 'Create' , 'uam_modules' , $uam_modules_id ,$input['module_name'], 'UAM Module has been Successfully Created' );
                    // $this->WriteFileLog(encrypt($uam_modules_id));
        ...
        try {
            ...
            if ($module_check == []) {
                ...
                DB::transaction(function () use ($input) {
                    ...
                    $this->admin_notification('UAM', 'Update' , 'uam_modules' , $input['module_id'] ,$input['module_name'], 'UAM Module has been Successfully Updated' );
    ...
    public function data_edit($id)
    {
        try {
            ...
            if ($id != 5) {
                ...
                $rows = DB::table('uam_modules')
                    ->select('*')
                    ->where([['active_flag', 0], ['parent_module_id', 0]])
                    ->get();
            } else {
                $rows = DB::table('uam_modules')
                    ->select('*')
    ...
    public function data_delete($id)
    {
        try {
            ...
            if ($check == []) {
                ...
                // $role_name_fetch=$role_name[0]->role_name;
                $this->auditLog('uam_modules', $id, 'Delete', 'Deleted the uam module', auth()->user()->id, NOW(), '');
