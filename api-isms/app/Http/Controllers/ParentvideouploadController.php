...
class ParentvideouploadController extends BaseController
{
    public function index(Request $request)
    {
        try {
            ...
            $pendingActivities = DB::table('performance_skill_activity')
                ->select('*')
                ->where('isVerified', 1)
                ->get();
    ...
    public function createdata()
    {
        //echo "naa";exit;
        try {
            $method = 'Method => ParentvideouploadController => createdata';
    ...
    {
        try {
            $method = 'Method => ParentvideouploadController => storedata';
    ...
    public function activityAction(Request $request)
    {
        try {
            ...
            DB::transaction(function () use ($input) {
                ...
                DB::table('performance_skill_activity')
    ...
    public function data_edit($id)
    {
        ...
        try {
            ...
            $method = 'Method => ParentvideouploadController => data_edit';
    ...
    public function data_edit_1($id)
    {
        try {
            ...
            $method = 'Method => ParentvideouploadController => data_edit_1';
    ...
    public function parent_createdata($id)
    {
        ...
        try {
            ...
            WHERE a.enableflag = 0 and a.activity_id=$activityID and b.user_id=$id AND(a.action_flag=0 OR a.action_flag IS NULL) ORDER BY a.activity_id,a.activity_description_id");

            $activitylist_nav = DB::select("SELECT save_flag , required , parent_video_upload_id , a.status , a.activity_description_id, a.status AS current_status , us.instruction,us.activity_description_id, us.description,act.activity_name,a.f2f_flag from parent_video_upload AS a 
            INNER JOIN enrollment_details AS b ON  b.enrollment_id=a.enrollment_id 
            INNER JOIN activity AS act ON act.activity_id=a.activity_id 
            INNER JOIN activity_description AS us ON us.activity_description_id=a.activity_description_id
            WHERE a.enableflag = 0 and a.status='New' and b.user_id=$id AND(a.action_flag=0 OR a.action_flag IS NULL)ORDER BY a.activity_id,a.activity_description_id");
            $activitylist_rejection = DB::select("SELECT save_flag , required , parent_video_upload_id,act.activity_name , a.status , a.activity_description_id, a.status AS current_status , us.instruction , us.description , a.f2f_flag from parent_video_upload AS a 
            INNER JOIN enrollment_details AS b ON  b.enrollment_id=a.enrollment_id 
            INNER JOIN activity AS act ON act.activity_id=a.activity_id 
            INNER JOIN activity_description AS us ON us.activity_description_id=a.activity_description_id
    ...
    {
        $logMethod = 'Method => ParentvideouploadController => Getdescription';
        ...
        try {
            ...
            foreach ($inputArray as $key => $value) {
                $id[$key] = DB::select("SELECT b.activity_id , a.activity_description_id , a.description , a.instruction , b.activity_name from activity_description AS a
    ...
    {
        try {
            $method = 'Method => ParentvideouploadController => parent_storedata';
            $inputArray = $this->DecryptData($request->requestData);
            $input = [
    ...
    public function video_reupload(Request $request)
    {
        try {
            $method = 'Method => ParentvideouploadController => video_reupload';
            $inputArray = $this->DecryptData($request->requestData);
            $input = [
                'video_link' => $inputArray['video_link'],
                'parent_video_upload_id' => $inputArray['parent_video_upload_id'],
                ...
                'current_status' => $inputArray['current_status'],
                'unable_flag' => $inputArray['unable_flag'],
                'rejectedReOpen' => $inputArray['rejectedReOpen'],
                'sameVideoLink' => $inputArray['sameVideoLink'],
            ];

            $enrollID = DB::transaction(function () use ($input) {
                $parent_video_upload = $input['parent_video_upload_id'];

                foreach ($parent_video_upload as $activityVideoID) {
                    $checkNewVideo = DB::select("select * from activity_parent_video_upload where parent_video_upload_id =" . $activityVideoID);
                    if (
                        count($input['video_link'][$activityVideoID]) != count($checkNewVideo) ||
                        (isset($input['unable_flag'][$activityVideoID]) && $input['unable_flag'][$activityVideoID] == "1") ||
                        (isset($input['sameVideoLink'][$activityVideoID]) && $input['sameVideoLink'][$activityVideoID] == "1")
                    ) {
                        $current_status = $input['current_status'][$activityVideoID];
                        $state = ($current_status == 'Rejected') ? 'Re-Sent' : 'Submitted';

                        DB::table('parent_video_upload')
                            ->where('parent_video_upload_id', $activityVideoID)
                            ->update([
                                'status' => $state,
                                // 'f2f_flag' => ($input['unable_flag'] == 1 ? '1' : '0'),
                                'last_modified_by' => auth()->user()->id,
                        ...
                        DB::select("Delete from activity_parent_video_upload where parent_video_upload_id =" . $activityVideoID);
                        foreach ($input['video_link'][$activityVideoID] as $video_link) {
                            DB::table('activity_parent_video_upload')
                                ->where('video_link_id', $input['video_link'][$activityVideoID])
                                ->insert([
                                    'video_link' => $video_link,
                                    'parent_video_upload_id' => $activityVideoID,
                                    'status' => '0',
                                    'created_by' => auth()->user()->id,
                                    'created_at' => NOW()
                                ]);
                        }
                        $initiateID = $activityVideoID;
                        $pvu = DB::select("SELECT * FROM parent_video_upload WHERE parent_video_upload_id = $initiateID");
                        $enID = $pvu[0]->Enrollment_id;
                        $enrol = DB::select("SELECT * FROM enrollment_details WHERE Enrollment_id = $enID");
                        ...
                        DB::table('latest_video_comments')->insertGetId([
                            ...
                            'created_by' => auth()->user()->id,
                            'created_at' => NOW(),
                            'comments' => $input['comments'][$activityVideoID],
                            'role' => $role_name[0]->role_name
                        ]);

                        $activity_description_id = $input['activity_description_id'][$activityVideoID];
                        $active_des = DB::select("SELECT * FROM activity_description AS ad INNER JOIN activity AS a ON a.activity_id=ad.activity_id
                        WHERE ad.activity_description_id=$activity_description_id");
                        $idIn = $pvu[0]->activity_initiation_id;
                        $rr = DB::select("SELECT * FROM parent_video_upload WHERE activity_initiation_id = $idIn AND STATUS = 'New'");
                        ...
                        $pvu = DB::select("SELECT * FROM parent_video_upload WHERE parent_video_upload_id = $activityVideoID");
                        $re = $pvu[0]->activity_initiation_id;
                        $enID = $pvu[0]->Enrollment_id;
                    }
                }
            ...
            $serviceResponse['Code'] = config('setting.status_code.success');
            $serviceResponse['Message'] = config('setting.status_message.success');
            $serviceResponse['Data'] = $rejectedReOpenID;
            $serviceResponse = json_encode($serviceResponse, JSON_FORCE_OBJECT);
            $sendServiceResponse = $this->SendServiceResponse($serviceResponse, config('setting.status_code.success'), true);
            return $sendServiceResponse;
        } catch (\Exception $exc) {
            ...
            $reid = DB::transaction(function () use ($input) {
                ...
                if ($input['submit_type'] == "Submit") {
                    ...
                    DB::table('notifications')->insertGetId([
                        ...
                        'notification_type' => 'activity',
                        'notification_status' => 'Updated',
                        'notification_url' => 'activity_initiate/' . encrypt($pvu[0]->activity_initiation_id) . '/edit?restoreActivity=' . $restoreActivityID,
    ...
    public function mapping_update(Request $request)
    {
        try {
            ...
            $method = 'Method =>  ParentvideouploadController => mapping_update';
    ...
    {
        try {
            $method = 'Method => ParentvideouploadController => policyaggrement';
    ...
    {
        $logMethod = 'Method => ParentvideouploadController => Getmaterials';

        try {
            $inputArray = $this->DecryptData($request->requestData);
            $enrollment_id = $inputArray['enrollment_id'];
            $description_id =  $inputArray['description_id'];

            $desc = DB::table('activity_materials')
                ->select('materials')
                ->get();

            $details = DB::table('parent_video_upload as pvu')
                ->join('enrollment_details as ed', 'pvu.enrollment_id', '=', 'ed.enrollment_id')
                ->where('ed.enrollment_child_num', $enrollment_id)
                ->where('activity_description_id', $description_id)
                ->select('pvu.parent_video_upload_id', 'pvu.comments')
                ->get();

            $f2f_Observation = collect();
            if ($details->isNotEmpty()) {
                $f2f_Observation = DB::table('face_to_face_observation')
                    ->where('parent_video_id', $details[0]->parent_video_upload_id)
                    ->get();
            }
    ...
    {
        $logMethod = 'Method => ParentvideouploadController => f2f_description';
        try {

            $inputArray = $this->DecryptData($request->requestData);
            $activity_id =  $inputArray['activity_id'];

            $data = DB::table('activity_description as a')
                ->join('activity as b', 'b.activity_id', '=', 'a.activity_id')
                ->where('b.activity_id', $activity_id)
                ->select('b.activity_id', 'a.activity_description_id', 'a.description', 'a.instruction', 'b.activity_name')
                ->get();
