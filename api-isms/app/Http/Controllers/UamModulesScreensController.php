...
class UamModulesScreensController extends BaseController
{
    ...
    public function getmodulesandscreens(Request $request)
    {
        ...
        try {
            ...
            $modulesdata = DB::select("select * from uam_modules where not parent_module_id = 0 and active_flag = 0  AND module_type=1" . $where_que);
    ...
    {
        try {
            $method = 'Method => UamModulesScreensController => get_sub_module';
            $inputArray = $this->decryptData($request->requestData);
            $module_id = $inputArray['module_id'];
            $user_id = auth()->user()->id;
            $sub_module = DB::select("SELECT * FROM uam_modules WHERE active_flag =0 AND parent_module_id = $module_id");
            // $this->WriteFileLog($sub_module);
            $response = [
    ...
    public function storedata(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($input) {
                ...
                $this->auditLog('uam_modules_screens', $screen_permission_id, 'Create', 'create uam module screen', auth()->user()->id, NOW(), $role_name_fetch);
                $mID = $input['module_id'];
                $na = DB::select("SELECT * FROM uam_modules WHERE module_id=$mID");$mName = $na[0]->module_name;
                $this->admin_notification('UAM', 'Create' , 'uam_modules_screens' , $input['module_id'] , $mName, ' UAM Module Screens has been Successfully Created' );
    ...
    public function data_delete($id)
    {
        try {
            ...
            if ($work_flow  == []) {
                ...
                $this->auditLog('uam_modules_screens', $id, 'Delete', 'Deleted the uam module screen', auth()->user()->id, NOW(), '');
