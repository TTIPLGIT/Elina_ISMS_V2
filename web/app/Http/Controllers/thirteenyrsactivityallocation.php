...
class thirteenyrsactivityallocation extends BaseController
{
    public function index()
    {
        try {
            ...
            //         $total = $parant_data['total'];
                    ...
                    return view('activity_allocation13.activitylist', compact('screens', 'modules', 'screen_permission'));
            //     }
            // } else {
            //     $objData = json_decode($this->decryptData($response->Data));
    ...
    public function create(Request $request)
    {
        ...
        try {
            $method = 'Method => thirteenyrsactivityallocation => create';
            ...
            //         $activity = $parant_data['activity'];
                    ...
                    $screens = $menus['screens'];
                    $modules = $menus['modules'];
                    return view('activity_allocation13.videoinitiate', compact('screens', 'modules'));
            //     }
            // } else {
            //     $objData = json_decode($this->decryptData($response->Data));
    ...
    {
        $method = 'Method => thirteenyrsactivityallocation => description';
        try {
            ...
            $gatewayURL = config('setting.api_gateway_url') . '/parentvideo13/description';
    ...
    {
        try { //dd($request);
            $method = 'Method => thirteenyrsactivityallocation => store';
            $data = array();
            ...
            if ($response1->Status == 200 && $response1->Success) {
                ...
                if ($objData->Code == 200) {
                    ...
                    return redirect(route('activity_allocation13.index'))->with('success', 'Activity Initiated Successfully');
                }
                if ($objData->Code == 400) {
    ...
    {
        $method = 'Method => thirteenyrsactivityallocation => activityajax';
        try {
    ...
    public function show($id)
    {
        ...
        try {
            ...
            $method = 'Method => thirteenyrsactivityallocation => show';
            ...
            if ($response->Status == 200 && $response->Success) {
                ...
                if ($objData->Code == 200) {
                    ...
                    $screens = $menus['screens'];
                    $modules = $menus['modules'];
                    return view('activity_allocation13.activityshow', compact('rows', 'screens', 'modules', 'activityshow'));
                }
            } else {
                $objData = json_decode($this->decryptData($response->Data));
                echo json_encode($objData->Code);
    ...
    {
        try {
            $method = 'Method => thirteenyrsactivityallocation => edit';
            ...
            //         $lastactivity1 = $parant_data['lastactivity1'];
                    ...
                    $menus = $this->FillMenu();
                    $screens = $menus['screens'];
                    $modules = $menus['modules']; //dd($currentactivity);

                    return view('activity_allocation13.activityedit', compact('screens', 'modules'));
            //     }
            // } else {
            //     $objData = json_decode($this->decryptData($response->Data));
            //     echo json_encode($objData->Code);
    ...
    public function update(Request $request, $id)
    {
        ...
        try { //dd($request , $id);
            $method = 'Method =>  thirteenyrsactivityallocation => update_data';
            $data = array();
    ...
    public function observation($id)
    {
        try {
            ...
            //         $comments = $parant_data['comments'];
                    ...
                    return view('activity_allocation13.activity_observation', compact('screens', 'modules'));
            //     }
            // } else {
            //     $objData = json_decode($this->decryptData($response->Data));
            //     echo json_encode($objData->Code);
    ...
    public function activeStatus(Request $request, $id)
    {
        ...
        try {
            ...
            if ($response1->Status == 200 && $response1->Success) {
                ...
                if ($objData->Code == 200) {
                    return redirect(route('activity_allocation13.index'))
                        ->with('success', 'Activity status Updated Successfully');
                }
                if ($objData->Code == 400) {
                    return Redirect::back()
    ...
    public function bulk_update(Request $request)
    {
        ...
        try { //dd($request);
            $method = 'Method =>  thirteenyrsactivityallocation => bulk_update';
            $data = array();
    ...
    public function update_toggle(Request $request)
    {
        try {
            ...
            $method = 'Method => thirteenyrsactivityallocation => update_toggle';
            $data = array();
    ...
    public function activity_save(Request $request)
    {
        try {
            ...
            // dd($request);
            $method = 'Method =>  thirteenyrsactivityallocation => activity_save';
            $data = array();
            $data['approval_status'] = $request->approval_status;
            $data['comments'] = $request->comments;
            $data['activity_initiation'] = $request->activity_initiation_id;
    ...
    {
        try { //dd($request);
            $method = 'Method =>  thirteenyrsactivityallocation => send_required';
            $data = array();
