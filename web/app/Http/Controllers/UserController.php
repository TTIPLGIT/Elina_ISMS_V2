   ...
   try {
      ...
      if($response1->Status == 200 && $response1->Success){
         ...
         if ($objData->Code == 200) {
             ...
             return redirect(route('user.index'))->with('success', 'Token Expire Settings Changed Successfully');
...
//return $request;
    ...
    try {
        ...
        $userRow['designation'] = $request->designation;
        ...
        $userRow['additional_roles_id'] = $request->additional_roles_id;
...

{
    try {
        ...
        $data['additional_roles_id'] = $request->additional_roles_id;
   ...
   try {
       ...
       if($response->Status == 200 && $response->Success){
           ...
           if ($objData->Code == 200) {
               ...
               $sub_module = $parant_data['sub_module'];
               $permissions_data =  $parant_data['permission_data'];
               $module_array = $parant_data['module_array'];
               ...
               return view('uam.user.edit_permission', compact('module_array','sub_module','parent_module_data','screens_data','modules','screens','module_data','permissions_data','user_id','rows','role_name'));
           }
       } 
