...
class AreasMasterController extends BaseController
{
    ...
    public function store_data(Request $request)
    {
        try {
            ...
            $data['recommended_environment'] = $request->description;
            $data['strategies_recommended'] = $request->file;
        ...
        try {
            ...
            if ($response->Status == 200 && $response->Success) {
                ...
                if ($objData->Code == 200) {
                    ...
                    $area_discription = $parant_data['area_discription'];
                    $menus = $this->FillMenu();
                    $screens = $menus['screens'];
                    $modules = $menus['modules'];
                    $permissions = config('permission.screen_permission');

                    return view('Recommendation_master.edit', compact('area_discription', 'permissions', 'area_edit', 'modules', 'screens'));
                }
            } else {
                $objData = json_decode($this->decryptData($response->Data));
                echo json_encode($objData->Code);
    ...
    public function update(Request $request)
    {
        try {
            ...
            $data['recommended_environment'] = $request->description;
            $data['strategies_recommended'] = $request->file;
            $data['new_recommended_environment'] = $request->new_description;
            $data['new_strategies_recommended'] = $request->new_file;
            $data['deleteInput'] = explode(',', $request->deleteInput);
            // dd($data);
            $encryptArray = $this->encryptData($data);
            $request = array();
            $request['requestData'] = $encryptArray;
    ...
    { //dd($this->decryptData($id));
        try {
            ...
            //  $id = $this->decryptData($id);
            $gatewayURL = config('setting.api_gateway_url') . '/area/master/delete/' . $id;
            $response = $this->serviceRequest($gatewayURL, 'GET', '', $method);
            $response1 = json_decode($response);
            ...
            if ($response1->Status == 200 && $response1->Success) {
                $objData = json_decode($this->decryptData($response1->Data));
                if ($objData->Code == 200) {
                    return redirect(route('areas_master.index'))->with('success', 'Data Deleted successfully!');
                } else {
                    return redirect(route('areas_master.index'))->with('fail', $objData->Code);
                }
            }
        } catch (\Exception $exc) {
