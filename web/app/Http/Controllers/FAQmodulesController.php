...
return view('FAQ.faq_modules.index', compact('rows','modules','screens','screen_permission','dynamiclistdata'));
}
}
} 
else {
...
$screens = $menus['screens'];
$modules = $menus['modules'];
return view('FAQ.faq_modules.create', compact('rows','modules','screens'));
}
}
else {
$objData = json_decode($this->decryptData($response->Data));
...
if ($objData->Code == 200) {
return redirect(route('faqmodules.index'))->with('success', 'FAQ Modules Created Successfully');
}
}
else {
$objData = json_decode($this->decryptData($response->Data));
...
$screens = $menus['screens'];
$modules = $menus['modules'];
return view('FAQ.faq_modules.edit', compact('rows','modules','screens'));
}
}
else {
$objData = json_decode($this->decryptData($response->Data));
...
$screens = $menus['screens'];
$modules = $menus['modules'];
return view('FAQ.faq_modules.show', compact('rows','modules','screens'));
}
}
else {
$objData = json_decode($this->decryptData($response->Data));
...
if ($objData->Code == 200) {
return redirect(route('faqmodules.index'))->with('success', 'FAQ Modules Updated Successfully');
}
