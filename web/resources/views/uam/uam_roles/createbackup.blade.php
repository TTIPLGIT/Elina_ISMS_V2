            ...
            <div class="col-md-12">
              <div id="treeview_container" class="hummingbird-treeview well h-scroll-large">
                ...
                <ul id="treeview" class="hummingbird-base">
                 ...
                 <li>
                  <i class="fa fa-plus"></i> <label> <input id="node-{{ $parent_module_data_value->module_id }}" data-id="{{ $parent_module_data_value->module_id }}" type="checkbox" module="{{ $parent_module_data_value->module_id }}"> {{ $parent_module_data_value->module_name }} </label> <ul>
                  @if($module_data !="")
                  @foreach ($module_data as $module_data_value)
                  @if($parent_module_data_value->module_id == $module_data_value->parent_module_id)
                 
                    <li><i class="fa fa-plus"></i> <label> <input id="node-{{$module_data_value->parent_module_id }}-{{$module_data_value->module_id }}" data-id="{{$module_data_value->parent_module_id }}-{{$module_data_value->module_id }}"  module="{$module_data_value->parent_module_id }}" type="checkbox"> {{$module_data_value->module_name }}</label><ul>

                      @if($screens_data !="")
                      @foreach ($screens_data as $screen_data_value)
                      @if($module_data_value->module_id == $screen_data_value->module_id)
                      <li> <label><input  id="node-{{$screen_data_value->module_id }}-{{$screen_data_value->screen_id }}" data-id="{{$screen_data_value->module_id }}-{{$screen_data_value->screen_id }}"  type="checkbox"> {{$screen_data_value->screen_name }} </label><ul>
...
///$screen_id = [];
 ...
 $.ajax({
  ...
  type:"POST",
  dataType:"json",
  data: {screen_id : $screen_id,role_name:$role_name,permission_id: $permission_id,_token: '{{csrf_token()}}' },
