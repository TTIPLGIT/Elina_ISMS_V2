...
<style>
  ...
  /* ==========================================
     MOBILE RESPONSIVE – CARD STYLE (same as modules index)
     ========================================== */
  @media (max-width: 768px) {
    ...
    /* Screen Name – primary field (2nd column) */
    ...
    /* Expanded fields */
    #align tr.expanded-row td:nth-of-type(3) { /* Route URL */
      display: block !important;
...
<div class="main-content">
  <section class="section">
    <div class="section-body mt-2">
      <!-- “Add Screens” button with margin-top: 50px -->
      @if(strpos($screen_permission['permissions'], 'Create') !== false)
        <a type="button" style="font-size:15px; margin: 50px 0px 5px 15px;" class="btn btn-success btn-lg" href="{{ route('uam_screens.create') }}">Add Screens</a>
      @endif
      <style>
        .section {
          margin-top: 20px;
                    ...
                    <tbody>
                      @foreach($rows as $key=>$row)
                        <tr>
                          ...
                          <td data-label="Screen Name">{{ $row['screen_name'] }}</td>
                          <td data-label="Route URL">{{ $row['route_url'] }}</td>
                          ...
                          <td data-label="Action" class="text-center">
                            <form action="{{ route('uam_screens.destroy', \Crypt::encrypt($row['screen_id'])) }}" method="POST">
                              @if(strpos($screen_permission['permissions'], 'Show') !== false)
                                <a class="btn btn-info btn-sm" href="{{ route('uam_screens.show', \Crypt::encrypt($row['screen_id'])) }}">Show</a>
                              @endif

                              @if(strpos($screen_permission['permissions'], 'Edit') !== false)
                                <a class="btn btn-warning btn-sm" href="{{ route('uam_screens.edit', \Crypt::encrypt($row['screen_id'])) }}">Edit</a>
                              @endif
