...
<body>
      ...
      <div class="main-sidebar sidebar-style-2" style="background-color: {{ $modules['user_role'] == 'Child' ? '#2a0245' : 'rgb(0, 34, 102)' }} !important;">
        <aside id="sidebar-wrapper" style="background-color: {{ $modules['user_role'] == 'Child' ? '#2a0245' : 'rgb(0, 34, 102)' }} !important;">
          ...
          <ul class="sidebar-menu" id="sidechecks">
            @if(!in_array($modules['user_role'], ['Parent', 'Child', '']))
            ...
            @php
            $uniqueScreens = explode(',', $screens['unique1'][0]['un']);
            $uniqueScreens2 = explode(',', $screens['unique2'][0]['un']);
            $checkedModules = [];
            @endphp

            @foreach ($screens['unique'] as $mscreen)
            @php
            if ($mscreen['parent_module_id'] == 1) {
            $msID = in_array($mscreen['module_id'], $uniqueScreens) ? 0 : $mscreen['module_id'];
            } else {
            $msID = $mscreen['parent_module_id'];
            if (in_array($msID, $checkedModules)) {
            $msID = 0;
            } else {
            $checkedModules[] = $msID;
            }
            ...
            <li class="dropdown">
              <a class="nav-link has-dropdown"><i class="{{ $module['class_name'] }}"></i><span>{{ $module['module_name'] }}</span></a>
              <ul class="dropdown-menu active" style="display: none;">
                ...
                @foreach ($modules['sub_module'] as $sub_module)
                @if($sub_module['parent_module_id'] == $module['module_id'] && in_array($sub_module['module_id'], $uniqueScreens2))
                <li class="dropdown">
                  <a class="nav-link has-dropdown"><span>{{ $sub_module['module_name'] }}</span></a>
                  <ul class="dropdown-menu active" style="display: none;">
                    @foreach ($screens['screens'] as $subscreen)
                ...
                @foreach ($screens['screens'] as $screen)
                @if($module['module_id'] == $screen['module_id'])
                <li><a class="nav-link smn" href="{{ config('setting.base_url') }}{{ $screen['route_url'] }}">{{ $screen['screen_name'] }}</a></li>
                @endif
                @endforeach
                @endif
              </ul>
  ...
  @include('layouts.reports_script')
