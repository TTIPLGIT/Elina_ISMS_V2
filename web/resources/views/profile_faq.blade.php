               ...
               <h1><b><u>Frequently Asked Questions on ELINA ISMS</u></b></h1>         
                  ...
                  <div id="faq_id">
                       ...
                       @foreach($one_row as $data)
                        <button type="button"  class="collapsible"><p id="name"><b> {{$loop->iteration}}){{ $data['module_name'] }}</b></p></button>
                        ...
                        @foreach ($rows as $key => $faqdata)
                          @if($data['id'] == $faqdata['module_id'])
                             <p id="question"><span style="padding:50px !important;"><b> Q{{$loop->iteration}}.{{$faqdata['question']}}</b></span></p>
