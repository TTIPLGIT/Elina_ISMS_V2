...
<div class="main-content">
    ...
    <div class="section-body mt-1">
        ...
        <form action="{{route('ovmiscfeedback_update',$rows[0]['ovm_isc_report_id'])}}" method="POST" id="ovmisc" name="ovmisc" enctype="multipart/form-data">
            {{ csrf_field() }}
            @method('PUT')
            <div class="row">
                <div class="col-12">
                            ...
                            <div class="row is-coordinate">
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input type="hidden" class="form-control" name="meet" placeholder="meet" value="{{$rows[0]['ovm_meeting_id']}}" readonly>
                ...
                <div class="col-12" style="margin: 5px 0 5px 0;cursor:pointer">
                    <nav class="nav" style="width:100%">
                        ...
                        @if($value['id'] !== 1)
                        <a class="nav-item{{ $value['id'] === 2 ? ' is-active' : '' }}" active-color="orange" data-id="{{$value['id']}}">{{$value['name']}}</a>
                        @endif
                        @endforeach
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                $coordinatorFeedback = DB::select("SELECT
                                JSON_UNQUOTE(JSON_EXTRACT(is_coordinator1, '$.name')) as coordinator_name1,
                                JSON_UNQUOTE(JSON_EXTRACT(is_coordinator1, '$.id')) as coordinator_id,
                                JSON_UNQUOTE(JSON_EXTRACT(is_coordinator2, '$.id')) as coordinator_id1,
                                JSON_UNQUOTE(JSON_EXTRACT(is_coordinator2, '$.name')) as coordinator_name2
                                FROM ovm_meeting_details WHERE enrollment_id = ?", [$enrollmentId]);

                                // Fetch coordinator's name from users table

                                $coordinatorNames = $coordinatorFeedback[0];
                                @endphp
                                ...
                                <div class="table-wrapper">
                                    <div class="table-responsive">
                                        <table class="table myTable">
                                            ...
                                            <thead>
                                                <tr>
                                                    <th width="15%" rowspan="2">Areas</th>
                                                    <th rowspan="2">IS-Coordinatoor's</th>
                                                    <th width="35%" rowspan="2">Conversation summary</th>
                                                </tr>
                                            </thead>
                                            ...
                                            <tbody id="table_{{$value['id']}}">
                                                ...
                                                <input type="hidden" name="que[{{ $rows[0]['ovm_isc_report_id'] }}][user_id]" value="que{{ $coordinatorNames->coordinator_id }}">
                                                    ...
                                                    <td width="35%"><textarea class="form-control default instructions_textarea {{ $question['assigned_value']}}" id="{{$question['question_column_name']}}" name="que[{{ $rows[0]['ovm_isc_report_id'] }}][{{ $question['question_column_name'] }}]">{{ $question['prefilled_data']}}</textarea>
                                                    </td>
                                                ...
                                                <input type="hidden" name="que[{{ $rows1[0]['ovm_isc_report_id'] }}][user_id]" value="{{ $coordinatorNames->coordinator_id1 }}">
                                                    ...
                                                    <td width="35%">
                                                        <textarea class="form-control default instructions_textarea 2_{{ $question['assigned_value']}}" id="2_{{$question['question_column_name']}}" name="que[{{ $rows1[0]['ovm_isc_report_id'] }}][{{ $question['question_column_name'] }}]">{{ $question['prefilled_data']}}</textarea>
                                                    </td>
                                            ...
                                            <tbody id="table_{{$value['id']}}">
                                                ...
                                                <input type="hidden" name="que[{{ $rows[0]['ovm_isc_report_id'] }}][user_id]" value="{{ $coordinatorNames->coordinator_id }}">
                                                    ...
                                                    <td style="text-align:left !important;">{!! $question['prefilled_data'] !!}<input type="hidden" value="{{ $question['prefilled_data'] }}" name="que[{{ $rows[0]['ovm_isc_report_id'] }}][{{ $question['question_column_name'] }}]"></td>
                                            ...
                                            <tbody id="table_{{$value['id']}}">
                                                ...
                                                <input type="hidden" name="que[{{ $rows[0]['ovm_isc_report_id'] }}][user_id]" value="{{ $coordinatorNames->coordinator_id }}">
                                                    ...
                                                    <td width="45%" style="height: 60px !important;"><textarea style="height: 60px !important;" class="form-control default instructions_textarea_readonly {{ $question['assigned_value']}}" id="{{$question['question_column_name']}}" name="que[{{ $rows[0]['ovm_isc_report_id'] }}][{{ $question['question_column_name'] }}]" disabled>{{ $question['prefilled_data']}}</textarea></td>
                                                    @else
                                                    <td width="45%"><textarea class="form-control default instructions_textarea {{ $question['assigned_value']}}" id="{{$question['question_column_name']}}" name="que[{{ $rows[0]['ovm_isc_report_id'] }}][{{ $question['question_column_name'] }}]">{{ $question['prefilled_data']}}</textarea></td>
                                                    @endif
                                                ...
                                                <input type="hidden" name="que[{{ $rows1[0]['ovm_isc_report_id'] }}][user_id]" value="{{ $coordinatorNames->coordinator_id1 }}">
                                                    ...
                                                    @if(isset($question['readonly']) && $question['readonly'] == 1)
                                                    <td style="height: 60px !important;"><textarea style="height: 60px !important;" class="form-control default instructions_textarea_readonly {{ $question['assigned_value']}}" id="2_{{$question['question_column_name']}}" name="que[{{ $rows1[0]['ovm_isc_report_id'] }}][{{ $question['question_column_name'] }}]" disabled>{{ $question['prefilled_data']}}</textarea></td>
                                                    @else
                                                    <td><textarea class="form-control default instructions_textarea {{ $question['assigned_value']}}" id="2_{{$question['question_column_name']}}" name="que[{{ $rows1[0]['ovm_isc_report_id'] }}][{{ $question['question_column_name'] }}]">{{ $question['prefilled_data']}}</textarea></td>
                                                    @endif
            ...
            <div class="row text-center">
                <div class="col-md-12">
                    ...
                    <button type="submit" id="submit" class="btn btn-success" name="type" value="Completed">Update</button>
                    @endif
                    <a type="button" href="{{ route('ovmreport') }}" class="btn btn-danger">Cancel</a>
