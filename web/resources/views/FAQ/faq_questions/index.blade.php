...
{{ Breadcrumbs::render('FAQ_questions.index')}}
  <section class="section">
    ...
    @if(strpos($screen_permission['permissions'], 'Create') !== false)
      <a type="button" style="font-size:15px; margin: 0 0px 5px 15px;" class="btn btn-success btn-lg"  href="{{ route('FAQ_questions.create') }}" >Create</a>
      @endif
        <style>
          .section {
            margin-top: 20px;
              ...
              <div class="table-wrapper">
                <div class="table-responsive">
                  ...
                  <form action="{{ route('uam_modules_screens.destroy', \Crypt::encrypt($row ['module_id'])) }}" method="POST">
                           ...
                           <a class="btn btn-info" href="{{ route('FAQ_questions.show', \Crypt::encrypt($row ['id'])) }}">{{ __('Show') }}<span></span></a>
                           @endif
                           @if(strpos($screen_permission['permissions'], 'Edit') !== false)
                                <a class="btn btn-warning"  href="{{ route('FAQ_questions.edit', \Crypt::encrypt($row['id'])) }}">{{ __('Edit') }}<span></span></a>
                                 @endif
                           @csrf
                           @method('DELETE') 
