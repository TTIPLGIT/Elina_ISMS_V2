                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="text-center">
                          ...
                          <a class="btn btn-info" href="{{ route('faqmodules.show', \Crypt::encrypt($row['id'])) }}">{{ __('Show') }}<span></span></a>
                          @endif
                          @if(strpos($screen_permission['permissions'], 'Edit') !== false)
                          <a class="btn btn-warning" href="{{ route('faqmodules.edit', \Crypt::encrypt($row['id'])) }}">{{ __('Edit') }}<span></span></a>
                          @endif

                          @if(strpos($screen_permission['permissions'], 'Delete') !== false)

                          <input type="hidden" name="delete_id" id="<?php echo $row['id']; ?>" value="{{ route('faqmodules.delete', \Crypt::encrypt($row['id'])) }}">

                          <a class="btn btn-danger" href="{{ route('faqmodules.delete', \Crypt::encrypt($row['id'])) }}" style="cursor: pointer; color:aliceblue !important;" onclick="return myFunction(<?php echo $row['id']; ?>);">{{ __('Delete') }}<span></span></a>
